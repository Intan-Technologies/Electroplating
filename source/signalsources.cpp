...
SignalSources::SignalSources()
{
    ...
    signalPort[0].name = L"Port A";
    signalPort[1].name = L"Port B";
    signalPort[2].name = L"Port C";
    signalPort[3].name = L"Port D";
    signalPort[4].name = L"Board ADC Inputs";
    signalPort[5].name = L"Board Digital Inputs";
    signalPort[6].name = L"Board Digital Outputs";

    signalPort[0].prefix = L"A";
    signalPort[1].prefix = L"B";
    signalPort[2].prefix = L"C";
    signalPort[3].prefix = L"D";
    signalPort[4].prefix = L"ADC";
    signalPort[5].prefix = L"DIN";
    signalPort[6].prefix = L"DOUT";
...
SignalChannel* SignalSources::findChannelFromName(const wstring &nativeName)
{
    SignalChannel *channel;

    for (unsigned int i = 0; i < signalPort.size(); ++i) {
...
BinaryWriter& operator<<(BinaryWriter& outStream, const SignalSources &a) 
{
    outStream << (int16_t) a.signalPort.size();
    for (unsigned int i = 0; i < a.signalPort.size(); ++i) {
...
BinaryReader& operator>>(BinaryReader &inStream, SignalSources &a)
{
    int16_t tempQint16;
    int nGroups;
